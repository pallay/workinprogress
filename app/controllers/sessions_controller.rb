...
class SessionsController < ApplicationController
  ...
  def create # RESTful login
    logout_keeping_session!
    user = User.authenticate(params[:login], params[:password])
    if user
      # Protects against session fixation attacks, causes request forgery protection
      # if user resubmits an earlier form using back button.
      # reset_session # Uncomment if you understand above tradeoff
